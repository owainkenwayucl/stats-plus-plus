...
(setv binaries { 
		...
		"unknown code 11"  ["docc_2d"]
		"unknown code 12"  ["negf3t"]
		"yambo"            ["yambo"]
		...
		"n2p2"             ["nnp-train"]
                "openmx"           ["openmx"]
		"mpi_pi"           ["mpi_pi.default" "mpi_pi.gcc" "mpi_pi"]
		"plumed"           ["plumed"]
		"pantarhei"        ["pantarhei.exe"]
                "zacros"           ["zacros.x"]
		})
...
(setv launchers ["export ASE_GULP_COMMAND" "export MDR" "export CMD" "CMD=" "export PW" "PW=" "EXEC=" "export EXEC" "UNRES_BIN" "chemsh" "g16" "g09" "star" "cfx5solve" "fluent" "R " "Rscript " "python" "matlab" "gmx" "USPEX" "mcsqs" "time" "./ramses2gadget" "abaqus" "CFD-SOLVER" "mdrun" "/usr/bin/time" "$HOME/bin" "${HOME}/bin" "${HOME}/src" "$HOME/src" "./calypso.x" "ansysdt" "wfl" "chemsh" "time" "APP="])
...
(defn getparaline[filename]
	...
	(try 
		(with [f (open filename)]
			(try
				...
				(except [UnicodeDecodeError]
					(do 
						(setv r "")
						(setv unidentified (+ "UnicodeDecodeError: " filename))
					)
				)
			)
		...
		(except [FileNotFoundError] 
			(do 
				(setv r "")
				(setv unidentified (+ "FileNotFoundError: " filename))
			)
		)
	)
	(if DEBUG_MODE (if (= r "") (print unidentified)))
	r
)
...
(defn getusagebyapp [service period]
	...
	; file wanted is $scriptloc/YYYY-MM-DD/$JOB_ID
	(for [a dbdata]
		(setv id (get a "job_number"))
		(setv seconds (* (get a "cost") (- (get a "end_time")(get a "start_time"))))
		(setv label (matchapp (getparaline (setfname service (+ (.strftime (datetime.datetime.fromtimestamp (get a "submission_time")) "%Y-%m-%d") "/" (str (get a "job_number")))))))
