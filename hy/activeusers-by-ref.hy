...
(defn getusageref [service monthlist refs debug] 
	...
	(setv query (simpletemplate.templatefile
				:filename "sql/activeusers-for-refs.sql"
				:keys keys))
...
(defn printCSV [monthlist refs data seperator debug] 
	...
	(print "")
	(print "Period" seperator "Active Users")
	(for [i (range nmonths)]
		(setv index (+ i 1))
		...
		(for [a refs]
          		(setv value 0.0)
          		(for [b data]
				(if (and (= (get b "Period") pperiod) (= (get b "ref_category") a)) (setv value(get b "Users")))
          		)
...
(defn getrefcats [debug]
	...
	(if debug (dbtools.log (str retval) "Categories found"))
	retval
)
...
;; Defaults
	...
	(setv parser (argparse.ArgumentParser :description "Generate acive user count for a named list of REF categories."))
